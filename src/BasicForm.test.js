...
describe("BasicForm", () => {
  ...
  test("submits the form when all fields are valid", async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith("http://localhost:5000/submit", expect.objectContaining({
        method: "POST",
        headers: {
    ...
    expect(screen.queryByText(/Phone must be 10 digits/i)).not.toBeInTheDocument();
